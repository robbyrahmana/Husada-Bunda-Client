...
@Component({
  templateUrl: './doktertreatment.site.html'
})
export class DokterTreatmentSite extends ListHelper implements List {

    // Variable
    dataPage: PaginationPage<PatientTreatment>;
    self: DokterTreatmentSite; 
    ...
    constructor(globalConstant: GlobalConstant, router: Router, dataService : PatientService) {
        ...
        this.generateConfig("doktermanagertreatment");
        let observable: Observable<PaginationPage<PatientTreatment>> = this.fetchPage(0, 10, null, "49ebc9cf-6a42-491c-a782-41c0d2ba9a66");
        this.self = this;
    }
    ...
    fetchPage(pageNumber: number, pageSize: number, sort: PaginationPropertySort, filter1: string = null, filter2: string = null, filter3: string = null, filter4: string = null, filter5: string = null): Observable<PaginationPage<any>> {
        let observable: Observable<PaginationPage<any>> = this.dataService.getPatientTreatmentAllTableByDokterId(pageNumber, pageSize, sort, filter1);
