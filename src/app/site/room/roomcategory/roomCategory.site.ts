...
export class RoomCategorySite extends ListHelper implements List {

    // Variable
    dataPage: PaginationPage<RoomCategory>;
    self: RoomCategorySite; 
    ...
    constructor(globalConstant: GlobalConstant, router: Router, dataService : RoomService) {
        ...
        let observable: Observable<PaginationPage<RoomCategory>> = this.fetchPage(0, 10, null, "", "");
        this.self = this;
    }
    ...
    fetchPage(pageNumber: number, pageSize: number, sort: PaginationPropertySort, filter1: string = null, filter2: string = null, filter3: string = null, filter4: string = null, filter5: string = null): Observable<PaginationPage<any>> {
        let observable: Observable<PaginationPage<any>> = this.dataService.getRoomCategoryAllTable(pageNumber, pageSize, sort, filter1, filter2);
