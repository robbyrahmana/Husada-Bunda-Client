...
@Component({
    selector: "patient-operation",
    templateUrl: './operationdetails.site.html'
})
export class PatientOperationDetailsSite extends ListHelper implements List {

    data: PatientOperation = new PatientOperation();
    tempid : string = "";
    // Variable
    dataPage: PaginationPage<PatientOperationDetails>;
    self: PatientOperationDetailsSite; 
    ...
    constructor(globalConstant: GlobalConstant,  router: Router, dataService : PatientService, private route: ActivatedRoute) {
        ...
        this.route.params.subscribe(params => {
            if (params['id'] != null) {
                this.dataService.getPatientOperationByID(params['id']).subscribe(data => {  
                    this.data = data;
                });
                let observable: Observable<PaginationPage<PatientOperationDetails>> = this.fetchPage(0, 10, null, params['id']);
            }
        });
    }
    ...
    fetchPage(pageNumber: number, pageSize: number, sort: PaginationPropertySort, filter1: string = null, filter2: string = null, filter3: string = null, filter4: string = null, filter5: string = null): Observable<PaginationPage<any>> {
        let observable: Observable<PaginationPage<any>> = this.dataService.getPatientOperationDetailsAllTable(pageNumber, pageSize, sort, filter1);
