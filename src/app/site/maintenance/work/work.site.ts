...
import { Work }                                     from '../../../service/domain/maintenance/maintenance.domain';
...
export class WorkSite extends ListHelper implements List {

    // Variable
    dataPage: PaginationPage<Work>;
    self: WorkSite; 
    ...
    constructor(globalConstant: GlobalConstant, router: Router, dataService : MaintenanceService) {
        ...
        this.generateConfig("work");
        let observable: Observable<PaginationPage<Work>> = this.fetchPage(0, 10, null, "");
        this.self = this;
    }
    ...
    fetchPage(pageNumber: number, pageSize: number, sort: PaginationPropertySort, filter1: string = null, filter2: string = null, filter3: string = null, filter4: string = null, filter5: string = null): Observable<PaginationPage<any>> {
        let observable: Observable<PaginationPage<any>> = this.dataService.getWorkAllTable(pageNumber, pageSize, sort);
