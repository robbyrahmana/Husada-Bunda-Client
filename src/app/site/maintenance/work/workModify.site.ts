...
@Component({
  templateUrl: './workModify.site.html'
})
export class WorkModifySite extends EditHelper implements Edit{
    
    dataContainer: Work = new Work();
    work: any;
    workid: any;
    ...
    constructor(globalConstant: GlobalConstant, route: ActivatedRoute, router: Router, dataService : MaintenanceService) {
        ...
        this.route.params.subscribe(params => {
            if (params['id'] != null) {
                ...
                this.newEntry = false;
                this.dataService.getWorkByID(params['id']).subscribe(data => {
    ...
    submit() {
        this.error = null;
        let observable:Observable<Work>;
        if (this.newEntry) {
            delete this.dataContainer.id;
            observable = this.dataService.saveWorkRecord(this.dataContainer);
        } else {
            observable = this.dataService.updateWorkRecord(this.dataContainer);
        }
