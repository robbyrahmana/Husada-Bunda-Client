...
export class GenderSite extends ListHelper implements List {

    // Variable
    dataPage: PaginationPage<Gender>;
    self: GenderSite; 
    ...
    constructor(globalConstant: GlobalConstant, router: Router, dataService : MaintenanceService) {
        ...
        let observable: Observable<PaginationPage<Gender>> = this.fetchPage(0, 10, null, "");
        this.self = this;
    }
    ...
    fetchPage(pageNumber: number, pageSize: number, sort: PaginationPropertySort, filter1: string = null, filter2: string = null, filter3: string = null, filter4: string = null, filter5: string = null): Observable<PaginationPage<any>> {
        let observable: Observable<PaginationPage<any>> = this.dataService.getGenderAllTable(pageNumber, pageSize, sort);
